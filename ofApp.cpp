...
void ofApp::update() {
	...
	for (int i = 0; i < this->location_list.size(); i++) {
		...
		this->setHexagonToMesh(this->face, this->frame, this->location_list[i], 30, 5);
	}
...
void ofApp::setHexagonToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float radius, float height) {
	...
	auto noise_param = ofNoise(glm::vec4(location * 0.015, ofGetFrameNum() * 0.005));
	...
	if (noise_param < 0.45) {
		...
		face_radius *= ofMap(noise_param, 0.45, 0.55, 0, 1);
	}
